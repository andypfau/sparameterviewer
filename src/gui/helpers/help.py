...
def show_help(doc: str = 'main.md'):
    try:
        ...
        error_dialog('Unable to locate documentation', 'Unable to locate documentation.', f'Try to locate <sparameterviewer/docs> yourself ({ex}).')
    
    try:
        open_file_in_default_viewer(str(helpfile_path))
    except Exception as ex:
        error_dialog('Unable to show documentation', f'Unable to show documentation.', f'Try to open <{str(helpfile_path)}> yourself ({ex}).')
